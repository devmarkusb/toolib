...
private:
    ...
    const too::math::ChartAxisProjSetup* setup_{nullptr}; // dyn-casted aux. alias of original setup member variable
...
void ChartAxisProj<QuValueType>::constr_impl() {
    this->setup_ = dynamic_cast<const too::math::ChartAxisProjSetup*>(ChartAxis<QuValueType>::setup_.get());
...
Chart2D<QuValueTypeX, QuValueTypeY>::Chart2D(
    ...
    const auto x_is_as_proj_axis_setup = dynamic_cast<const too::math::ChartAxisProjSetup*>(&setup_x);
    ...
    const auto y_is_as_proj_axis_setup = dynamic_cast<const too::math::ChartAxisProjSetup*>(&setup_y);
