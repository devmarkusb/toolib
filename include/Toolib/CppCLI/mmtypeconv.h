...
namespace too
{
	namespace CLI
	{
		...
		template<typename T> inline void array2cpp_readonly(array<T>^ a, T*& converted_a)
		{
			pin_ptr<T> pa = &a[0];
			converted_a = pa;
		}
		template<typename T> inline void array2cpp(array<T>^ a, T*& converted_a, size_t size)
		{
			Marshal::Copy(a, 0, static_cast<IntPtr>(converted_a), size);
		}
		template<typename T> inline array<T>^ array2cli(T*& a,  size_t size)
		{
			array<T>^ converted_a = gcnew array<T>^(size);
			Marshal::Copy(static_cast<IntPtr>(a), converted_a, 0, size);
			return converted_a;
		}
		template<typename T> inline array<IntPtr>^ array2cli(T**& a,  size_t size)
		{
