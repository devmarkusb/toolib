...
namespace too
{
	...
	*/
	...
	public:
		scope_flagged() {}
		//! Default copy and assignment are ok. Doesn't make much sense, but shouln't be forbidden to not not spoil derivates.
		...
		struct SG_flags : public toobit::bits<bit_container_type>
		{
			explicit SG_flags(bit_container_type flags = m_flags->get())
				...
				{ m_auxLastflags = m_flags; m_flags = this; }
			~SG_flags() { m_flags = m_auxLastflags; }

			//! Default copy and assignment are ok. Doesn't make much sense, but shouln't be forbidden to not spoil derivates.
	...
	typename scope_flagged<user, bit_container_type>::SG_flags
		scope_flagged<user, bit_container_type>::
		m_init
		(user::SF_DEF);
	template<class user, class bit_container_type>
	typename scope_flagged<user, bit_container_type>::SG_flags*
