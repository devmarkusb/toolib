	...
	namespace con
	{
		...
		public:
			...
			void setWndSize(SHORT x, SHORT y)
			{
				...
				cWidthHeight.X = bufSize.X; cWidthHeight.Y = bufSize.Y;
				rLTRB.Left = wndSize.Left; rLTRB.Top = wndSize.Top;
				rLTRB.Right = wndSize.Right; rLTRB.Bottom = wndSize.Bottom;
			...
			void CopyDblBuffer2Screen(long pause)
			{
				...
				BOOL fSuccess = WriteConsoleOutputA(
					hOutput, dblbuf, cWidthHeight, cLeftTop, &rLTRB);
				if (fSuccess == FALSE)
				{
		...
		private:
			...
			{
				return cWidthHeight.X * cWidthHeight.Y;
			}
